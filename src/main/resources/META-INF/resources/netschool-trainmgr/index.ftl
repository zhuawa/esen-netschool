...
require(["eui/modules/etree","eui/modules/epanelsplitter", "eui/modules/etabctrl"], function(obj, obj1, etabctrl){
    ...
    var treedata = [
        {id:"level11", caption:"培训班管理", level: 1},
        {id:"level12", caption:"培训资源管理", level: 1},
    ];
    //模拟数据创建
    ...
    rootitem.loadFromArray(treedata, function(item, userObj){
        ...
        if(level === 2)return;
        for(var i = 0; i < 2; i++){
        	if(userObj.id == 'level12'){
        		if(i==0){
        			data.push({id:("level"+ level + i), caption:("场地管理"), level: level+ 1});
        		}else if(i==1){
        			data.push({id:("level"+ level + i), caption:("讲师管理"), level: level+ 1});
        		}
        	}
        }
	...
	function showTab(itemId){
		if(itemId=='level11'){
			queryPage(0, "培训班管理", "/page/rms/learnresourcesmgr");
		}else if(itemId=='level12'){
			queryPage(1, "培训资源管理", "/page/rms/learnresourcesmgr");
		}
	}
