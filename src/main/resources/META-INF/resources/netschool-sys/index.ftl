...
require(["eui/modules/etree","eui/modules/epanelsplitter", "eui/modules/etabctrl"], function(obj, obj1, etabctrl){
    ...
    var treedata = [
        {id:"level11", caption:"用户权限", level: 1},
        {id:"level12", caption:"系统设置", level: 1}
    ];
    //模拟数据创建
    ...
    rootitem.loadFromArray(treedata, function(item, userObj){
        ...
        for(var i = 0; i < 2; i++){
        	if(userObj.id == 'level11'){
        		data.push({id:("level"+ level + i), caption:("课件目录"+ i), level: level+ 1});
        	}else if(userObj.id == 'level12'){
        		data.push({id:("level"+ level + i), caption:("资源目录"+ i), level: level+ 1});
        	}
        }
	...
	function showTab(itemId){
		if(itemId=='level11'){
			queryPage(0, "课件管理", "/page/rms/learnresourcesmgr");
		}else if(itemId=='level12'){
			queryPage(1, "学习资源管理", "/page/rms/learnresourcesmgr");
		}else if(itemId=='level13'){
			queryPage(2, "新闻管理", "/page/rms/learnresourcesmgr");
		}else if(itemId=='level14'){
			queryPage(3, "文库管理", "/page/rms/learnresourcesmgr");
		}
	}
