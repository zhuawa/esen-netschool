...
require(["eui/modules/etree","eui/modules/epanelsplitter", "eui/modules/etabctrl"], function(obj, obj1, etabctrl){
    ...
    var treedata = [
        {id:"level11", caption:"我的考试", level: 1},
        {id:"level12", caption:"试题管理", level: 1},
        {id:"level13", caption:"题库管理", level: 1},
        {id:"level14", caption:"试卷管理", level: 1}
    ];
    //模拟数据创建
    ...
    rootitem.loadFromArray(treedata, function(item, userObj){
        ...
        if(level === 2)return;
        for(var i = 0; i < 1; i++){
        	if(userObj.id == 'level13'){
        		data.push({id:("level"+ level + i), caption:("题库列表"), level: level+ 1});
        		data.push({id:("level"+ level + i), caption:("知识点列表"), level: level+ 1});
        		data.push({id:("level"+ level + i), caption:("标签列表"), level: level+ 1});
        	}
        }
	...
	function showTab(itemId){
		if(itemId=='level11'){
			queryPage(0, "我的考试", "/page/rms/examquestionmgr");
		}else if(itemId=='level12'){
			queryPage(1, "试题管理", "/page/rms/examquestionmgr");
		}else if(itemId=='level13'){
			queryPage(2, "题库管理", "/page/rms/questionbankmgr");
		}else if(itemId=='level14'){
			queryPage(3, "试卷管理", "/page/rms/testpapermgr");
		}
	}
	...
	queryPage(0, "我的考试", "/page/rms/learnresourcesmgr");
	//设置打开的标签页
	tabobj.setActive(0);
